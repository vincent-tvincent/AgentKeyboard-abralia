...
Host Interaction capabilities response and are checked against the supplied
profile. The profile supplies device identity and the reserved toggle; RGB
...
python3 experiments/host-interaction-control-inspector/inspect_controls.py --profile builtin:keychron-v3-8k-ansi-encoder-effect25
```
...
matrix address and encoder direction, excluding the firmware-reserved toggle
position declared by `interaction.toggle_matrix` in the supplied profile. It uses MIRROR routing and emits DOWN events only, so normal
