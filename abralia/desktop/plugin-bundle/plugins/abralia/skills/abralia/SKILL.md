---
name: abralia
description: Enable this task in Abralia when the user explicitly asks to use the agent keyboard, then report progress, attention and optional keymap guides.
---

# Abralia keyboard

Use Abralia only after the user explicitly asks to enable or use it for this task,
for example “Enable Abralia for this task” or an explicit `$abralia` invocation.
Installing the plugin, enabling a project in the app, an ordinary coding request,
or merely seeing Abralia tools is not a request to activate a task. Do not activate
another agent on your own initiative.

Once requested, permission persists for this task through its work and follow-up
questions until the user asks to stop or disables it in the app. Do not release
your slot before asking a question that should notify the user. A later disabled
project or user-released slot is not an invitation to repair/re-enable it; require
a new explicit request before enabling it again.

Use `enable_self` to enable your verified current project if necessary and claim
your own slot. The app must be running for slot allocation; an offline result may
save project enrollment without acquiring a slot. Report that distinction and
continue the main task. Ordinary hooks and status checks never enable projects.

The app combines enabled projects on one keyboard. Your workspace is verified from native Codex task context; never supply or guess another project path. The backend
...
- After the explicit activation request, call `enable_self` with a concise task label, unique
  ...
  This enables the current project when necessary and allocates your slot in one call. Desktop registration
...
- If already registered for this user-authorized task, keep that allocation and
  use `get_status`; do not repeat activation on every turn. `acquire_slot` remains
  available for registration/recovery within an enabled project, under the same
...
If tools are missing, the project is not enabled, native project context cannot be verified, or tools return `skipped` because the backend/device is unavailable,
